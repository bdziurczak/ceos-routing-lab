...
- `clab-ceos-routing-lab/` — containerlab state and per-node directories:
	- `ansible-inventory.yml` — inventory for Ansible
	- `authorized_keys` — SSH keys file used by nodes
	- `topology-data.json` — containerlab topology runtime data
	- `<node>/flash/` — per-node flash directory containing `startup-config`, `kickstart-config`, and other device files which persist across redeploys
